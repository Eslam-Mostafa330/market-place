...
class PasswordResetController extends BaseApiController
{
    ...
    public function forgotPassword(ForgotPasswordRequest $request): JsonResponse
    {
        ...
        if (! $wasLinkSent) {
            return $this->apiResponse([], __('auth.too_many_requests'), 429);
        }

        return $this->apiResponse([], __('auth.reset_link_sent'), 200);
    }
    ...
    public function resetPassword(ResetPasswordRequest $request): JsonResponse
    {
        ...
        if (! $emailForReset) {
            return $this->apiResponse([], __('auth.reset_link_invalid'), 400);
        }

        return $this->apiResponse([], __('auth.password_reset_success'), 200);
    }
}